...
def menu_main(message):
  ...
  if decision not in options:
    mixer.Sound("menu_invalid.wav").play()
    menu_main("Invalid Option")
    return

  else:
    if options[decision] == "Save Data":
      mixer.Sound("menu_enter.wav").play()
      for i in range(1,11):
          lineskip()
      ...
      input("\nBack>")
      mixer.Sound("menu_back.wav").play()
      ...
      mixer.Sound("menu_enter.wav").play()
      player_stats = {'name': 'Navee', 'weapon': 'Gold Sword', 'LVL': 1, 'MEXP': 200, 'EXP': 0, 'MHP': 250, 'HP': 250, 'STR': 100, 'RES': 0, 'CRIT': 5, 'EVA': 5, 'buff': {}, 'debuff': {}, 'fusion': ['Wind Type 01', 'Landing CRIT hits regenerate 1% HP'], 'moves': {'Lunge': 0,"Garu" : 0, "Dia" : 0}, "weakness" : "Fire"}
    ...
    if options[decision] == "Learn Skills":
      ...
      while True:
        ...
        #Enough cards required and there is space in Player move list
          ...
          mixer.Sound("menu_skilllearnt.wav").play()
      ...
      while True:
        ...
        if decision.lower() == "back" or decision.lower() == "b":
          ...
          mixer.Sound("menu_fusionlearnt.wav").play()
...
def getfusion(fusion):
  ...
  ,"Fire Type 02" : "When in Critical condition, regenerate 2% HP"
  ...
  "Water Type 09": "Effects of Buffs and Debuffs are doubled.",

  "Prototype: AQUA": "Use the might of AQUA, gaining the power to manipulate currents: When taking a hit, applies the 'Awakened' buff to self, increasing all Elemental damage by 60% and increasing RES by 60%. The effects of any Buffs on self are doubled in Critical condition."}]
...
def printplus(title, auto, speed, text):
    ...
    for letter in text:
        ...
        if letter in symbols_all:
            ...
            time.sleep(speed)
            if (count % 3) == 0: mixer.Sound("menu_text.wav").play()
            output += letter
            print("\n" * 50)
            print(output)
            count += 1
...
def battle_neutral(actor, action, target):
  ...
  while hits > 0:
    ...
    if battle_dmgmiss(actor,target) <= 10:
      ...
      if outputtemp == "":
        mixer.Sound(sounds[battle_turndata["actionelement"]]).play()
      #Critical hit sound effect
      else:
        mixer.Sound(sounds_crit[battle_turndata["actionelement"]]).play()
  ...
  if action[4] != "None" and action[3] == "Offence":
    ...
    mixer.Sound("hit_debuff.wav").play()
